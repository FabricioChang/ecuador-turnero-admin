...
-- Actualizar categorías existentes sin identificador usando un CTE
...
SET identificador = 'CAT-' || LPAD(nr.rn::TEXT, 3, '0')
...
-- Actualizar publicidad existente sin identificador usando un CTE
WITH numbered_rows AS (
  SELECT id, ROW_NUMBER() OVER (ORDER BY created_at) as rn
  FROM public.publicidad
  WHERE identificador IS NULL
)
UPDATE public.publicidad p
SET identificador = 'PUB-' || LPAD(nr.rn::TEXT, 3, '0')
FROM numbered_rows nr
WHERE p.id = nr.id;
...
-- Actualizar custom_roles existentes sin identificador usando un CTE
WITH numbered_rows AS (
  SELECT id, ROW_NUMBER() OVER (ORDER BY created_at) as rn
...
SET identificador = 'ROL-' || LPAD(nr.rn::TEXT, 3, '0')
FROM numbered_rows nr
WHERE cr.id = nr.id;
